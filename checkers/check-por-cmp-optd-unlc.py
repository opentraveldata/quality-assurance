...
if __name__ == '__main__':
  ...
  with open (optd_por_extd_unlc_file, newline='') as csvfile:
    ...
    for row in file_reader:
      ...
      #
      if not por_code in optd_por_dict or optd_por_dict[por_code] != '':
  ...
  with open (optd_por_unlc_file, newline='') as csvfile:
    file_reader = csv.DictReader (csvfile, delimiter='^')
    for row in file_reader:
      unlc_por_code = row['por_code']
      #unlc_por_name = row['por_name']
      ...
      unlc_por_exc = False
      if unlc_por_code in por_exc_dict:
        if "I" in por_exc_dict[unlc_por_code]['source'] and por_exc_dict[unlc_por_code]['actv_in_optd'] == "0" and por_exc_dict[unlc_por_code]['actv_in_src'] == "1":
          unlc_por_exc = True

          # Remove the record from the known errors, as it has been handled
          por_exc_dict[unlc_por_code]['used'] = True

      # Check whether the UN/LOCODE derived POR is in the list of OPTD POR
      if not unlc_por_code in optd_por_dict and not it_cty_code in optd_por_dict and not unlc_por_exc:
      ...
      elif not unlc_por_exc:
        # Still not a known exception, but the UN/LOCODE POR is known by OPTD,
        # either as a travel-related POR or as a city POR:
        # if unlc_por_code in optd_por_dict or it_cty_code in optd_por_dict

        it_code = unlc_por_code
        if unlc_por_code in optd_por_dict:
          # First, register the UN/LOCODE derived POR, so that we can then,
          # in the next main loop, search for OPTD POR not referenced by UN/LOCODE
          unlc_por_dict[unlc_por_code] = {'loc_type': it_loc_type,
        ...
        if it_cty_code in optd_por_dict:
          ...
          unlc_por_dict[it_cty_code] = {'loc_type': "C",
                                      'state_code': it_state_code,
                                      'country_code': it_ctry_code,
                                      'city_code': it_cty_code}
          # Only the UN/LOCODE city code is known by OPTD
          if not unlc_por_code in optd_por_dict:
            it_code = it_cty_code

        # The OPTD POR is in the list of UN/LOCODE derived POR,
        ...
        if optd_env_id != '' and it_ctry_code != optd_ctry_code:
          ...
          # in the list of UN/LOCODE derived POR
          reasonStr = "UN/LOCODE derived POR no longer valid in OpenTravelData"
          reportStruct = {'por_code': unlc_por_code, 'in_optd': 1, 'in_unlc': 1,
                          'env_id': optd_env_id,
        ...
        if optd_env_id == '' and not unlc_por_code in optd_por_dict:
          # Only the UN/LOCODE city code is known by OPTD
          reasonStr = "UN/LOCODE derived POR in OpenTravelData as a city, but not as travel-related POR"
          reportStruct = {'por_code': unlc_por_code, 'in_optd': 1, 'in_unlc': 1,
                          'env_id': optd_env_id,
                          'date_from': optd_date_from,
                          'date_until': optd_date_until,
        ...
        if it_state_code != optd_state_code:
          # The state codes are not the same for the UN/LOCODE- and OPTD-derived POR
          ...
          if not is_state_exc:
            ...
            reasonStr = "UN/LOCODE- and OPTD-derived POR have not the same state codes"
            reportStruct = {'por_code': unlc_por_code, 'in_optd': 1, 'in_unlc': 1,
                            'env_id': optd_env_id,
                            'date_from': optd_date_from,
                            'date_until': optd_date_until,
  ...
  for optd_por_code in optd_por_dict:
    ...
    if not optd_por_code in unlc_por_dict and optd_env_id == "" and not optd_por_exc_in_optd_but_not_it:
      ...
      reportStruct = {'por_code': optd_por_code, 'in_optd': 1, 'in_unlc': 0,
                      'optd_geo_id': optd_geo_id,
                      'optd_state_code': optd_state_code,
                      'optd_ctry_code': optd_ctry_code,
                      'optd_cty_list': optd_cty_list_str,

...
if __name__ == '__main__':
  ...
  ## Output
  # OPTD points of reference (POR) with multiple cities, among which one
  # is not referenced as a city in OPTD POR public file
  output_por_city_not_in_optd_file = 'results/optd-qa-por-city-not-in-optd.csv'
  optd_por_city_not_in_optd_list = [('iata_code', 'optd_pk', 'loc_type',
                                     'geo_id', 'city_code')]
      ...
      if (len(optd_bksf_city_code_list_str) > 3 ):
          ...
          for optd_city_code in optd_city_code_array:
              ...
              if not optd_city_code in optd_por_dict:
                  ...
                  # If the IATA code of the city cannot be found,
                  # report the issue in a distinct list (and CSV file)
                  if not 'city' in optd_por_record_dict:
                      reportStruct = (optd_bksf_iata_code, optd_bksf_pk,
                                      optd_bksf_loc_type, optd_bksf_geo_id,
                                      optd_city_code)
                      optd_por_city_not_in_optd_list.append (reportStruct)
          
  ## Write the output lists into CSV files
  # OPTD points of reference (POR) with multiple cities, among which one
  # is not referenced as a city in OPTD POR public file
  with open (output_por_city_not_in_optd_file, 'w', newline ='') as csvfile:
    file_writer = csv.writer (csvfile, delimiter='^')
    for record in optd_por_city_not_in_optd_list:
